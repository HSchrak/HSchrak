...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec 12 07:56:10 2023
...
def bad_visibility(visibility,minimum):
    ...
    units = visibility_data.get("units")

    if units == "FT":
        minimum_vis = visibility_data.get("minimum", 0) / 5280
        return minimum > minimum_vis

    if units == "SM":
        prevailing_vis = visibility_data.get("prevailing", 0)
        return minimum > prevailing_vis
...
def bad_winds(winds,maxwind,maxcross):
    ...
    speed = windData.get("speed",0)
    crosswind = windData.get("crosswind",0)
    gusts = windData.get("gusts",0)
    units = windData.get("units")
    ...
    max_speed = max(speed, gusts)

    if max_speed > maxwind or crosswind > maxcross:
        return True
...
def bad_ceiling(ceiling,minimum):
    ...
    for layer in ceiling:
        type_data = layer.get("type")
        height = layer.get("height", 0)

        if type_data in ["broken", "overcast", "indefinite ceiling"]:
            if height < minimum:
                return True
...
def get_weather_report(takeoff,weather):
    ...
    closest_time = None
    for timestamp in weather:
        timestamp_dt = utils.str_to_time(timestamp)
        if timestamp_dt < takeoff:
            closest_time = utils.str_to_time(closest_time)
            if closest_time is None or timestamp_dt > closest_time:
                # print(takeoff, timestamp_dt, closest_time)
                closest_time = timestamp
            else:
                break

    return weather.get(closest_time.isoformat(), None)
...
def get_weather_violation(weather,minimums):
    ...
    if weather == None:
        return 'Unknown'
    if bad_ceiling(weather.get('sky'), minimums[0]):
        violations.append('Ceiling')
    if bad_visibility(weather.get('visibility'), minimums[1]):
        violations.append('Visibility')        
    if bad_winds(weather.get('wind'), minimums[2],minimums[3]):
        violations.append('Winds')
...
def list_weather_violations(directory):
    ...
    # Result list to store violations
    violations = []
# For each lesson
    for row in lessons[1:]:
        pilotID,airplaneID,instructor,takeoff,landing,filed,area = row 
        for student in students:
            if pilotID in student:
                break

        
        # Get the takeoff time
        takeoff = utils.str_to_time(takeoff)
        # Get the pilot credentials
        creds = pilots.get_certification(takeoff, student)
        ...
        if utils.daytime(takeoff, daycycle):
            daytime = True
        else:
            daytime = False
        mini = pilots.get_minimums(creds, area, instructed, vfr, daytime, minimums)
        # Get the weather conditions
        weatherReport = get_weather_report(takeoff,weather)

        # Check for a violation and add to result if so
        viol = get_weather_violation(weatherReport,mini)
        if viol != '':
            row.append(viol)
